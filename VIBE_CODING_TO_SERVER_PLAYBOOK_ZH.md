...
访问地址：`http://ai-hub.uat.shopee.io`
...
```mermaid
flowchart LR
    U["公司用户的浏览器"] -->|"访问 ai-hub.uat.shopee.io"| D["公司 DNS 或内部网络"]
...
4. 页面如何访问后端？同域 `/api/growth-recommendation`。
...
Nginx 监听服务器 80 端口，将所有页面和 API 请求代理到应用容器：
...
- `127.0.0.1/api/health` 验证 Nginx 到 Node 的代理。
...
服务器已有公司内部域名 `ai-hub.uat.shopee.io`，因此复用该域名访问 Nginx 的 80 端口。没有在项目仓库中管理公司 DNS、证书或 SSO 配置。
...
curl -fsS http://ai-hub.uat.shopee.io/api/health
```

然后用浏览器打开：

```text
http://ai-hub.uat.shopee.io
```
...
curl -fsS -X POST \
  http://ai-hub.uat.shopee.io/api/growth-recommendation \
  -H 'Content-Type: application/json' \
  --data '{"sellerName":"深圳市菲德越科技有限公司 - GGP"}'
```
...
```sh
curl -fsS http://ai-hub.uat.shopee.io/api/initial-data
```
...
- 是否发出了 `POST /api/growth-recommendation`。
- 请求体是否包含 `sellerName`。
- HTTP 状态码是什么。
