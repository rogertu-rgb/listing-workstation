...
Nginx :80 (or company Ingress)
  ├─ GET  /listing_workstation/api/initial-data
  └─ POST /listing_workstation/api/growth-recommendation
             │
             ▼
       Node.js API service
...
- 浏览器和 Nginx 入口统一增加 `/listing_workstation` 前缀，例如 `GET /listing_workstation/api/health`。
- Node 容器内部仍使用 `GET /api/health`、`GET /api/initial-data`、`POST /api/growth-recommendation` 和 `POST /api/admin/refresh-sellers`。
- 推荐接口请求体为 `{"sellerName":"..."}`，直接查询 DataSuite 并返回页面数据。
- 手动刷新接口使用 `Authorization: Bearer <ADMIN_TOKEN>`。
...
2. `/listing_workstation/api/health` 的 `dataSuiteConfigured` 为 `true`。
3. `/listing_workstation/api/initial-data` 返回卖家数组和 `updatedAt`。
